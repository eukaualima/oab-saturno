...
module.exports = 
{
	...
	async execute(interaction, client) 
    {
        ...
        pool.query(`SELECT * FROM mandados WHERE codigo = ${codigo}`, async function (erro, processo)
        {
            ...
            pool.query(`UPDATE mandados SET status = "Indeferido" WHERE codigo = ${codigo}`);
            ...
            await interaction.channel.edit({ name: `[Fechado][Indeferido] mandado#${codigo}` });
            ...
            .setColor(cor_embed)
            .addFields([
                { name: `<:oab_veredito:1187577594472837171> | Veredito`, value: `Mandado **INDEFERIDO**.` },
